...
const register = async (req, res) => {
  try {
    ...
    const { email, password, user_role_id } = req.body;
    if (!email || email == "") {
      res.json({ status: "0", message: "email is required" });
      return;
    }
    if (!user_role_id || user_role_id == "") {
      res.json({ status: "0", message: "user_role_id is required" });
      return;
    }
    ...
    if (user) {
      ...
      bcrypt.hash(password, saltRounds, async function (err, hash) {
        ...
        try {
          ...
          let newUser = await USERS_MODEL.findOne({ _id: user._id }).populate("user_role_id")
...
const login = async (req, res) => {
  try {
    ...
    let errors_arr = []
    if (!email || email == "") {
      errors_arr.push("Email")
    }
    if (!password || password == "") {
      errors_arr.push("Password")
    }


    if (errors_arr.length != 0) {
      res.json({ status: "0", message: errors_arr.join(" and ") + " is required" });
      return;
    }
...
const update = async (req, res) => {
  const { userId } = req.userData;
  try {
    let response = await USERS_MODEL.findByIdAndUpdate(userId, req.body);
    res.json({
      status: "1",
...
const getAll = async (req, res) => {
  try {
    let response = await USERS_MODEL.find().populate("user_role_id").sort({ "createdAt": "-1" });
    res.json({
      status: "1",
...
const deleteOne = async (req, res) => {
  const { id } = req.params;
  try {
    let response = await USERS_MODEL.findByIdAndDelete(id);
    res.json({
      status: "1",
...
//   }
//   let main_logo = "https://www.learnglobal.com/schoollogo/logo.png"
